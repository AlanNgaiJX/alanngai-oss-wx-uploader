class Uploader {
  constructor(signatureApi, { maxSideWidth, quality }) {
    ...
    // 图片处理
    this.maxSideWidth = maxSideWidth || undefined; // 限制图片最长边大小，若有这个选项，则压缩图片。如 1080
  ...
  // 【 选择文件 】
  chooseFile({ count, sourceType, mediaType, sizeType }, callbacks) {
    this.choosing = true;
    wx.chooseMedia({
      count: count || 1,
    ...
    _this.checkQueueList = async function () {
      if (
        ...
        if (_this.maxSideWidth && !uploadTask.isCompressed) {
          try {
            const resultFilePath = await Uploader.compressImage(
              uploadTask.filePath,
              _this.maxSideWidth,
              _this.quality
            );
  ...
  retryErrorTaskById(taskId) {
    ...
    if (uploadTaskIndex > -1) {
      ...
      const { id, url, suffix, filePath, md5, isCompressed } = uploadTask;
      const _uploadTask = {
        id,
        url,
        suffix,
  ...
  // 压缩图片
  static compressImage(src, maxSideWidth, quality) {
    return new Promise((resolve, reject) => {
      wx.getImageInfo({
        src,
          ...
          wx.compressImage({
            ...
            success: ({ tempFilePath }) => {
              if (!tempFilePath) {
                ...
                tempFilePath.lastIndexOf(".") === tempFilePath.length - 1 &&
                  (_case = 3);
